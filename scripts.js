...
const lyricChangeReducer = (state = initialState.songsById, action) => {
  ...
  switch (action.type){
    case 'NEXT_LYRIC':
    let newArrayPosition = state[action.currentSongId].arrayPosition + 1;
...
const userClick = () => {
  ...
  if (store.getState().songsById[store.getState()].arrayPosition === store.getState().songsById[store.getState().currentSongId].length - 1) {
